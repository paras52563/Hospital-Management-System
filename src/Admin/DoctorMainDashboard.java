...
public class DoctorMainDashboard extends JFrame {
    ...
    public DoctorMainDashboard() {
        ...
        JPanel card2 = createCard("Doctor Management", "\u2699", e -> {
            dispose();
            new admin.UpdateDeleteDoctorApp().setVisible(true);
        });

        JPanel card3 = createCard("View Doctors", "\uD83D\uDC65", e -> {
           JDialog loading = new JDialog(this, "Loading", true);
    loading.setLayout(new BorderLayout());
    loading.add(new JLabel("Loading data, please wait...", SwingConstants.CENTER), BorderLayout.CENTER);
    loading.setSize(300, 120);
    loading.setLocationRelativeTo(this);

    // Run your heavy task in the background
    SwingWorker<Void, Void> worker = new SwingWorker<>() {
        @Override
        protected Void doInBackground() throws Exception {
            // Simulate or actually load data (DB query, etc.)
            Thread.sleep(1500); // simulate delay
            // You can also preload data from DB here
            return null;
        }

        @Override
        protected void done() {
            loading.dispose(); // close loading popup
            new DoctorDisplayModern().setVisible(true); // open next page
            dispose(); // close current page (optional)
        }
    };
    
    worker.execute();
    loading.setVisible(true); // show dialog (blocks UI only until disposed)
        });
